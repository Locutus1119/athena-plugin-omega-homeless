        ...
        <div class="content">
            ...
            <div class="items mt-4" v-for="(trashItem, index) in trashItems" :key="index">
                ...
                <Button
                    class="mt-4"
                    color="light-blue"
                    @click="removeItem(trashItems[index].name, index)"
...
export default defineComponent({
    ...
    methods: {
        ...
        closeMenu(identifier: string,) {
            alt.emit(`${ComponentName}:Close`, identifier);
        },
        removeItem(binItem: string, index: number) {
            this.trashItems.splice(binItem, 1);
            alt.emit(`${ComponentName}:Vue:RemoveItem`, binItem, index, this.binIdentifier);
        },
        // handleKeyPress(e) {
        //     // Escape Key
